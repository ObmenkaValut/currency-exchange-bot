...
export const AI_PROMPT_TEMPLATE = `Канал обмена валют/крипты/финансовых операций. 
ПРОПУСКАЙ: обмен/покупка/продажа валют и крипты (USD, EUR, UAH, BTC, USDT), обменники, котировки валют, криптовалютные операции, финансовые переводы (WeChat Pay, Alipay, банки и тп.), наличные операции, займы под крипту, бизнес-услуги с валютой и схожее.
БЛОКИРУЙ: курсы/обучение/менторство, разблокировки счетов, товары, спам, MLM, казино, бессмысленные символы, маты.
При сомнении - пропускай.

"{TEXT}"

Верни JSON. Причины(МАКСИМУМ 3 СЛОВА):
• не про обмен
• нецензурная лексика
{"allowed":true/false,"reason":"макс 3 слова"}`

//   `Канал обмена валют/крипты. Пропускай ТОЛЬКО если текст явно про покупку/продажу/обмен валют (USD, EUR, UAH, BTC, USDT и т.д.). Блокируй всё остальное: спам, продажу курсов, бессмысленные символы, нерелевантное, услуги разблокировок счетов и все схожее.
//
// "{TEXT}"
//
...
• ${FREE_DAILY_LIMIT} поста/день (${MAX_LENGTH_FREE} симв). 
• Эмодзи недоступны. 
• Ссылки запрещены.
